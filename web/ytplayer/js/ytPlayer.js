...
function handlePlayList(d) {
    ...
    for (var i in d['playlist']) {
        ...
        tableData += "<tr>" +
                     "<td width=\"15\"><div class=\"button\" onclick=\"deletePLSong('" + id + "')\"><i class=\"fa fa-trash-o\" /></div></td>" +
...
function handleSongList(d) {
    ...
    for (var i in d['songlist']) {
        ...
        tableData += "<tr>" +
                     "    <td width=\"15\"><div class=\"button\" onclick=\"deleteSong('" + id + "')\"><i class=\"fa fa-trash-o\" /></div></td>" +
                     "    <td width=\"15\"><div class=\"button\" onclick=\"stealSong('" + id + "')\"><i class=\"fa fa-bookmark\" /></div></td>" +
                     "    <td>" + title + "</td>" +
                     "    <td>" + requester + "</td>" +
                     "    <td>" + duration + "</td>" +
...
function handlePlay(id, title, duration, requester) {
    ...
    try {
        ...
        $("#currentSong").html('<div class="column center"></div><div class="column add-bottom"><div id="mainwrap"><div class="panelheader" id="nowPlay"><span class="left" id="npAction">' + title + '</span><span class="right" id="npTitle"><a id=\"playerLink\" href=\"" + ytLink + "\">' + ytLink + '</a></span></div><div id=\"songProgressBar\"></div><div id="audiowrap"><div id="audio0">' +
'<span class="left">Duration: ' + duration + '</span>'
                   + '<span class="right">Requested by ' + requester + '</span>' + '</div><div id="tracks"><a id="btnPrev" onclick=\"handlePause()\">&rsaquo;</a><a id="btnNext" onclick=\"skipSong()\">&raquo;</a></div></div><div id=\"songRequestDiv\"</div></div>');
