...
(function () {
        ...
        if (panelCheckQuery(msgObject, 'discord_commands')) {
            ...
            for (i in keys) {
                if (keys[i]['table'] == 'discordPermcom') {
                    ...
                    if (dataObj[keys[i]['key']] === undefined) {
                        dataObj[keys[i]['key']] = {
                            cooldown: JSON.parse(keys[i]['value']).seconds,
                            isGlobal: (JSON.parse(keys[i]['value']).isGlobal == 'true')
                        };
                    } else {
            ...
            for (i in keys) {
                if (keys[i]['table'] == 'discordCommands') {
                    ...
                    html += '<tr>' +
                        ...
                        '<td>' + cooldown + ' sec ' + '</td>' +
                        '<td><button type="button" class="btn btn-info btn-fill btn-xs" onclick="$.openCommandModal(\'' + command + '\', \'' + response + '\', \'' + permission + '\', \'' + cost + '\', \'' + cooldown + '\', \'' + channel + '\', \'' + global + '\')"><i class="fa fa-pencil" /> </button>' +
    ...
    function updateDiscordCommand(cmd, isToRemove) {
        if ((typeof isToRemove === 'string' ? isToRemove == 'true' : isToRemove == true)) {
            ...
            sendDBUpdate('discord_command', 'discordCooldown', command, JSON.stringify({
                command: String(command),
                seconds: String(cooldown),
                isGlobal: String(checked)
            }));
            ...
            setTimeout(function () {
                sendWSEvent('discord', './discord/commands/customCommands.js', null, [command, permission, channel, alias, price]);
            }, TIMEOUT_WAIT_TIME);
    ...
    function setDiscordSlotRewards() {
        ...
        if (val0.length > 0 && val1.length > 0 && val2.length > 0 && val3.length > 0 && val4.length > 0) {
            ...
            setTimeout(function () {
                doQuery();
                sendWSEvent('discord', './discord/games/slotMachine.js', null, null);
            }, TIMEOUT_WAIT_TIME);
        }
    }
    ...
    function setDiscordRollRewards() {
        ...
        if (val0.length > 0 && val1.length > 0 && val2.length > 0 && val3.length > 0 && val4.length > 0 && val5.length > 0) {
            ...
            setTimeout(function () {
                doQuery();
                sendWSEvent('discord', './discord/games/roll.js', null, null);
            }, TIMEOUT_WAIT_TIME);
        }
    }
