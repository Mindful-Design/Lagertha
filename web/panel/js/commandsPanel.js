...
(function () {
            ...
            if (panelCheckQuery(msgObject, 'commands_cooldown')) {
                ...
                for (idx in msgObject['results']) {
                    ...
                    html += '<tr >' +
                        ...
                        '            <input type="text" id="editCommandCooldown_' + commandName.replace(/[^a-zA-Z0-9_]/g, '_SP_') + '"' +
        ...
        if (panelCheckQuery(msgObject, 'commands_commands')) {
            ...
            for (var idx in msgObject['results']) {
                ...
                html += '<tr >' +
                    ...
                    '           <input type="text" class="form-control" id="editCommand_' + commandNameSafe.replace(/[^a-zA-Z0-9_]/g, '_SP_') + '"' +
                    '                   value="' + commandValue + '" />' +
                    ...
                    '           <button class="btn btn-danger btn-fill btn-xs" type="button" id="deleteCommand_' + commandNameSafe.replace(/[^a-zA-Z0-9_]/g, '_SP_') + '" onclick="$.deleteCommand(\'' + commandName + '\')""><i class="fa fa-trash" /></button>' +
                    '           </span>' +
                    '           </div>' +
                    '           </div>' +
                    '           </form>' +
        ...
        if (panelCheckQuery(msgObject, 'commands_aliases')) {
            ...
            for (idx in msgObject['results']) {
                ...
                html += "<tr >" +
                    ...
                    "    <td>!" + commandName + "</td>" +
                    "    <td>" +
                    "        <div id=\"deleteAlias_" + commandName.replace(/[^a-zA-Z0-9_]/g, '_SP_') + "\" type=\"button\" class=\"btn btn-danger btn-fill btn-xs\" " +
        ...
        if (panelCheckQuery(msgObject, 'commands_pricecom')) {
            ...
            for (idx in msgObject['results']) {
                ...
                html += '<tr >' +
                    ...
                    '       <div class="form-group form-group-table">' +
                    '       <div class="input-group">' +
                    '              <input type="text" class="form-control" id="editCommandPrice_' + commandName.replace(/[^a-zA-Z0-9_]/g, '_SP_') + '"' +
                    '                   value="' + commandValue + '" />' +
                    '              <span class="input-group-btn">' +
                    ...
                    '              <button type="button" class="btn btn-danger btn-fill btn-xs" id="deleteCommandPrice_' + commandName.replace(/[^a-zA-Z0-9_]/g, '_SP_') + '" onclick="$.deleteCommandPrice(\'' + commandName + '\')"><i class="fa fa-trash" /> </button>' +
                    '           </span>' +
                    '           </div>' +
                    '           </div>' +
                    '           </form>' +
        ...
        if (panelCheckQuery(msgObject, 'commands_payment')) {
            ...
            for (idx in msgObject['results']) {
                ...
                html += '<tr >' +
                    ...
                    '    <td>' +
                    '        <form onkeypress="return event.keyCode != 13">' +
                    '            <input  type="text" id="editCommandPay_' + commandName.replace(/[^a-zA-Z0-9_]/g, '_SP_') + '"' +
                    '                   value="' + commandValue + '" />' +
                    '              <button type="button" class="btn btn-danger btn-fill btn-xs" onclick="$.updateCommandPay(\'' + commandName + '\')"><i class="fa fa-pencil" /> </button> ' +
                    '              <button type="button" class="btn btn-default btn-fill btn-xs" id="deleteCommandPay_' + commandName.replace(/[^a-zA-Z0-9_]/g, '_SP_') + '" onclick="$.deleteCommandPay(\'' + commandName + '\')"><i class="fa fa-trash" /> </button>' +
                    '             </form>' +
                    '        </form>' +
                    '    </td>' +
                    '</tr>';
                ...
                if (commandName.indexOf(' ') === -1) {
                    if (disabledCommands[commandName] !== undefined) {
                        html += "<td><div id=\"commandEnabled_" + commandName + "\"" +
                            ...
                            "    <i  class=\"fa fa-toggle-off\" /></div></td>";
                    } else {
                        html += "<td><div id=\"commandEnabled_" + commandName + "\"" +
                            "         data-toggle=\"tooltip\" title=\"Disable Command\" class=\"button\" onclick=\"$.commandEnable('" + commandName + "', 'disable');\">" +
                            "    <i  class=\"fa fa-toggle-on\" /></div></td>";
                    }
                } else {
                    html += "<td />";
                }

                html += "<td /><td><div id=\"commandsList_" + commandName + "\"><strong><font >" + groupIcons[commandValue] +
                    "    </font></strong></div></td>" +
    ...
    function deleteCommand(command) {
        $("#deleteCommand_" + command.replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i  class=\"fa fa-spinner fa-spin \" / > ");
        ...
        setTimeout(function () {
            sendCommand("reloadcommand " + command);
        }, TIMEOUT_WAIT_TIME);
    };
    ...
    function deleteCommandPrice(command) {
        $("#deleteCommandPrice_" + replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i  class=\"fa fa-spinner fa-spin \" / > ");
    ...
    function deleteCommandPay(command) {
        $("#deleteCommandPay_" + command.replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i class=\"fa fa-spinner fa-spin fa-spin \" />");
    ...
    function deleteAlias(command) {
        $("#deleteAlias_" + command.replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i  class=\"fa fa-spinner fa-spin \" / > ");
        if (command.length != 0) {
            command = command.replace('!', '');
    ...
    function commandPermission(command, group) {
        $("#commandsList_" + command.replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i  class=\"fa fa-spinner fa-spin \" / > ");
    ...
    function updateCommandPrice(command) {
        ...
        $('#editCommandPrice_' + command.replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i  class=\"fa fa-spinner fa-spin \" / > ");
        if (val > 0) {
    ...
    function updateCommandPay(command) {
        ...
        $('#editCommandPay_' + command.replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i  class=\"fa fa-spinner fa-spin \" / > ");
        if (val.length > 0) {
     ...
     */
    function toggleCooldownMsg() {
        $("#cooldownMsg").html("<i  class=\"fa fa-spinner fa-spin \" / > ");
        if (cooldownMsg == "true") {
     ...
     */
    function togglePermcomMsg() {
        $("#permcomMsg").html("<i  class=\"fa fa-spinner fa-spin \" / > ");
        if (permcomMsg == "true") {
     ...
     */
    function toggleModCooldown() {
        $("#toggleModCooldown").html("<i  class=\"fa fa-spinner fa-spin \" / > ");
        if (modCooldown == "true") {
    ...
    function deleteCooldown(command) {
        $("#deleteCooldown_" + command.replace(/[^a-zA-Z0-9_]/g, '_SP_')).html("<i  class=\"fa fa-spinner fa-spin \" / > ");
    ...
    function addCooldown() {
        ...
        if (input.length > 0 && command.length != 0) {
            sendDBUpdate("commands_cooldown_add", "cooldown", String(command), JSON.stringify({
                command: String(command.toLowerCase()),
                seconds: String(input),
                isGlobal: String(checked)
            }));
            setTimeout(function () {
                doQuery();
                sendWSEvent('cooldown', './core/commandCoolDown.js', null, ['add', command.toLowerCase(), input, checked]);
            }, TIMEOUT_WAIT_TIME);
            setTimeout(function () {
                $("#cooldownCmdInputCommand").val("");
                $("#cooldownCmdInput").val("");
                $("#globalCooldownCheck").prop('checked', true);
            }, 5);
    ...
    function commandEnable(commandName, action) {
        if (panelMatch(action, 'enable')) {
            $('#commandEnabled_' + commandName).html('<i class="fa fa-toggle-on" />');
            sendDBDelete('commands_enablecom', 'disabledCommands', commandName);
            sendCommand('registerpanel ' + commandName);
        } else {
            $('#commandEnabled_' + commandName).html('<i class="fa fa-toggle-off" />');
